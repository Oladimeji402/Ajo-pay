...
function GroupCard({ group, selected, onSelect }: { group: (typeof groups)[0]; selected: boolean; onSelect: () => void }) {
  ...
  return (
    <motion.button
      ...
      onClick={onSelect}
      whileTap={{ scale: 0.98 }}
      className={`w-full text-left p-4 rounded-2xl border-2 transition-all duration-200 ${selected ? 'border-brand-emerald bg-emerald-50/50 shadow-sm shadow-brand-emerald/10' : 'border-slate-100 bg-white hover:border-slate-200 hover:shadow-sm'
        }`}
    >
...
export default function OnboardingPage() {
  ...
  const nextSlide = () => { if (slideIndex < slides.length - 1) { setSlideIndex(i => i + 1); } else { goTo('profile'); } };
  ...
  return (
    <div className="h-screen bg-[#F8FAFC] overflow-hidden flex flex-col">
      ...
      <header className="flex-shrink-0 flex items-center justify-between px-5 py-4 bg-white/80 backdrop-blur-xl border-b border-slate-100 z-10">
        ...
        <button onClick={() => router.push('/dashboard')} className="text-[13px] text-brand-gray hover:text-brand-navy font-medium transition-colors">
          Skip
        </button>
      </header>
          ...
          {phase === 'group' && (
            <motion.div key="group" custom={dir} variants={pageVariants} initial="enter" animate="center" exit="exit" transition={slide} className="absolute inset-0 flex flex-col">
              ...
              <div className="flex-shrink-0 px-5 pb-8 pt-4 bg-white/80 backdrop-blur-xl border-t border-slate-100 space-y-3">
                ...
                <GhostBtn onClick={() => router.push('/dashboard')}>Skip for now</GhostBtn>
              </div>
            </motion.div>
          )}
