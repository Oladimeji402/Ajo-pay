...
export const Features = () => {
  return (
    <section id="features" className="py-24 lg:py-32 bg-[#F8FAFC] relative overflow-hidden">
      <div className="absolute bottom-0 left-0 w-[400px] h-[400px] bg-brand-primary/[0.03] rounded-full blur-[80px] -ml-48 -mb-48 pointer-events-none" />
      ...
      <Container className="relative z-10">
        ...
        <div className="text-center max-w-2xl mx-auto mb-14">
          ...
          >
            <div className="inline-flex items-center gap-2 px-3.5 py-1.5 rounded-full bg-brand-primary/[0.08] border border-brand-primary/15 mb-5">
              ...
              <span className="text-[11px] font-bold text-brand-primary uppercase tracking-[0.1em]">Features</span>
            </div>
            <h2 className="text-3xl sm:text-4xl font-extrabold text-brand-navy mb-4 tracking-tight">
              Everything you need to{' '}
              <span className="text-transparent bg-clip-text bg-gradient-to-r from-brand-primary to-brand-electric">save smarter</span>
            </h2>
        ...
        <div className="grid lg:grid-cols-3 gap-4 lg:gap-5">
          ...
          >
            {/* BG glow */}
            <div className="absolute top-0 right-0 w-64 h-64 bg-brand-primary/[0.08] rounded-full blur-[80px] -mr-16 -mt-16 pointer-events-none" />
            ...
            <div className="relative z-10 flex flex-col sm:flex-row gap-8 items-start h-full">
              <div className="flex-1">
                <div className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full bg-brand-electric/15 border border-brand-electric/20 mb-4">
                  <Zap size={10} className="text-brand-electric" />
                  <span className="text-[10px] font-bold text-brand-electric uppercase tracking-wider">Automation</span>
                </div>
                  ...
                  {[
                    ...
                    <div key={i} className="flex items-center justify-between gap-3">
                      <div className="flex items-center gap-2">
                        <div className={`w-5 h-5 rounded-full flex items-center justify-center flex-shrink-0 ${row.status === 'done' ? 'bg-brand-electric/20' :
                            row.status === 'active' ? 'bg-brand-primary/20' : 'bg-white/[0.06]'
                          }`}>
                          {row.status === 'done' && <CheckCircle2 size={11} className="text-brand-electric" />}
                          {row.status === 'active' && <div className="w-2 h-2 rounded-full bg-brand-primary animate-pulse" />}
                          {row.status === 'upcoming' && <div className="w-2 h-2 rounded-full bg-white/20" />}
                        </div>
                        <span className="text-[11px] text-slate-400">{row.label}</span>
                      </div>
                      <span className={`text-[11px] font-bold ${row.status === 'done' ? 'text-brand-electric' : row.status === 'active' ? 'text-brand-primary' : 'text-slate-600'}`}>
                        {row.amount}
                      </span>
                    </div>
                  ))}
          ...
          >
            <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-56 h-56 bg-brand-primary/[0.10] rounded-full blur-[80px] pointer-events-none" />

            <div className="relative z-10 h-full flex flex-col">
              <div className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full bg-brand-electric/15 border border-brand-electric/20 mb-5 w-fit">
                <Shield size={10} className="text-brand-electric" />
                <span className="text-[10px] font-bold text-brand-electric uppercase tracking-wider">Security First</span>
              </div>

              {/* Shield icon centered */}
              <div className="flex justify-center mb-6">
                <div className="relative w-20 h-20 flex items-center justify-center">
                  <div className="absolute inset-0 bg-brand-electric/10 rounded-full animate-ping opacity-30" />
                  <div className="absolute inset-2 bg-brand-electric/15 rounded-full" />
                  <Shield size={36} className="text-brand-electric relative z-10" />
                </div>
              </div>
              ...
              <div className="space-y-3 mt-auto">
                {[
                  ...
                  <div key={i} className="flex items-center gap-2.5 p-2.5 bg-white/[0.04] rounded-xl border border-white/[0.06]">
                    <div className="text-brand-electric flex-shrink-0">{item.icon}</div>
          ...
          <motion.div
            ...
            className="rounded-3xl bg-gradient-to-br from-brand-primary/[0.06] to-brand-soft-blue border border-brand-primary/10 p-7 relative overflow-hidden group hover:-translate-y-1 hover:shadow-xl hover:shadow-brand-primary/10 transition-all duration-300 min-h-[200px]"
          >
            <div className="absolute top-0 right-0 w-32 h-32 bg-brand-primary/[0.08] rounded-full blur-[50px] -mr-8 -mt-8 pointer-events-none" />
            <div className="relative z-10">
              ...
              <div className="flex items-center gap-2 mb-1.5">
                ...
                <ArrowUpRight size={16} className="text-brand-primary opacity-0 group-hover:opacity-100 transition-opacity" />
              </div>
              ...
              {/* Payout visual */}
              <div className="mt-4 flex items-center gap-2 p-3 bg-white rounded-xl shadow-sm border border-brand-primary/10">
                <div className="w-7 h-7 bg-brand-primary/10 rounded-full flex items-center justify-center flex-shrink-0">
          ...
          >
            <div className="flex flex-col md:flex-row gap-8 items-center">
              ...
              <div className="flex-1">
                <div className="w-12 h-12 bg-brand-primary/10 rounded-2xl flex items-center justify-center mb-4 group-hover:scale-110 transition-transform duration-300">
                  <Smartphone size={22} className="text-brand-primary" />
                </div>
              ...
              <div className="flex-shrink-0 grid grid-cols-3 gap-3 w-full md:w-auto">
                {[
                  ...
                  { label: 'Debit Card', color: 'text-brand-primary', bg: 'bg-brand-soft-blue', icon: <CreditCard size={18} /> },
                  { label: 'USSD', color: 'text-brand-primary', bg: 'bg-brand-primary/[0.06]', icon: <Smartphone size={18} /> },
                ].map((method, i) => (
